...
(function () {
  ...
  const MARKET_EVENTS = [
    {
      ...
      color: "#FF5252",
      emoji: "📉",
      description: "Market crashed -20%!",
      duration: 0, // instant
      apply: (match) => {
        match.stockWorth = Math.round(match.stockWorth * 0.8 * 100) / 100;
        match.worthHistory.push(match.stockWorth);
      },
    },
    {
      type: "bull_run",
      ...
      color: "#27C47A",
      emoji: "🐂",
      description: "Buys are 3x more powerful for 20s!",
      duration: 20000,
      apply: null,
    },
    {
      ...
      color: "#FACC15",
      emoji: "⚡",
      description: "Extreme volatility for 15s!",
      duration: 15000,
      apply: null,
    },
    {
      ...
      color: "#C9913A",
      emoji: "💰",
      description: "Stock holders receive cash bonus!",
      duration: 0,
      apply: (match) => {
        Object.values(match.playerStates).forEach((ps) => {
          if (ps.stocks >= 3) ps.cash += 50;
        });
      },
    },
    {
      ...
      color: "#7C3AED",
      emoji: "🧊",
      description: "Trading halted for 15s!",
      duration: 15000,
      apply: null,
    },
    {
      ...
      color: "#22D3EE",
      emoji: "📊",
      description: "Stock price doubled!",
      duration: 0,
      apply: (match) => {
        match.stockWorth = Math.round(match.stockWorth * 2.0 * 100) / 100;
        match.worthHistory.push(match.stockWorth);
      },
    },
  ];
  ...
  function getMyTeamId() {
    ...
    if (state.teams[0].players.some((p) => p.id === state.myPlayerId))
      return "a";
    if (state.teams[1].players.some((p) => p.id === state.myPlayerId))
      return "b";
    return null;
  }
  ...
  async function handleSignalingMessage(payload) {
    ...
    const voiceMode =
      (state.match_settings && state.match_settings.voice_mode) || "team";
    ...
    try {
      if (payload.type === "join") {
        ...
        console.log(
          `[VOICE] join received from ${payload.senderId}. Am I initiator? ${shouldInitiate}`,
        );
        ...
        if (shouldInitiate) {
          ...
          if (!pcExists) {
            console.log(
              `[VOICE] I am non-initiator. Sending join reply to ${payload.senderId}`,
            );
        ...
        if (pc) {
          if (pc.remoteDescription) {
            ...
            if (!webrtc.candidateQueues[payload.senderId])
              webrtc.candidateQueues[payload.senderId] = [];
            webrtc.candidateQueues[payload.senderId].push(
  ...
  function createPeerConnection(targetId, isInitiator) {
    if (webrtc.peers[targetId]) {
      ...
      if (iceState !== "failed" && iceState !== "closed") {
        console.log(
          `[VOICE] peer already exists for ${targetId} (${iceState})`,
        );
        return existing;
      }
      // Dead peer — replace it
      console.log(
        `[VOICE] replacing dead peer for ${targetId} (was: ${iceState})`,
      );
      existing.close();
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        console.log(
          `[VOICE] Gathered candidate type: ${event.candidate.type} (${protocol})`,
        );
        broadcastSignal({
          type: "candidate",
          targetId: targetId,
    ...
    pc.oniceconnectionstatechange = async () => {
      ...
      console.log(
        `[VOICE] ICE connection state change for ${targetId}: ${pc.iceConnectionState}`,
      );
      ...
      if (pc.iceConnectionState === "disconnected") {
        if (amIInitiator) {
          setTimeout(async () => {
            ...
            if (
              ...
              console.log(
                `[VOICE] ICE still disconnected for ${targetId}, restarting...`,
              );
        ...
        if (amIInitiator) {
          ...
          console.log(
            `[VOICE] ICE failed for ${targetId}, closing and reconnecting...`,
          );
          closePeerConnection(targetId);
          setTimeout(() => {
            if (
              !webrtc.peers[targetId] &&
              !webrtc.isMuted &&
              webrtc.localStream
            ) {
          ...
          console.log(
            `[VOICE] ICE failed for ${targetId}. Waiting for initiator to reconnect.`,
          );
        }
      }
      ...
      if (
        ...
        try {
          ...
          if (activeCandidatePair) {
            ...
            if (local && remote) {
              console.log(
                `[VOICE] Selected candidate pair for ${targetId}: Local(${local.candidateType}) <-> Remote(${remote.candidateType}) via ${local.relayProtocol || "direct/stun"}`,
              );
            }
          }
        } catch (e) {
    ...
    pc.onsignalingstatechange = () => {
      console.log(
        `[VOICE] Signaling state change for ${targetId}: ${pc.signalingState}`,
      );
    };
  ...
  function initApp() {
    ...
    $("landing-join-btn").addEventListener("click", () => {
      dom.landing.hidden = true;
    ...
    if (dom.switchTeamBtn)
      dom.switchTeamBtn.addEventListener("click", switchTeam);
    ...
    if (disbandRoomBtn)
      disbandRoomBtn.addEventListener("click", initiateDisbandVote);
    if (arenaDisbandBtn)
      arenaDisbandBtn.addEventListener("click", initiateDisbandVote);

    const disbandAgreeBtn = $("disband-agree-btn");
    const disbandDisagreeBtn = $("disband-disagree-btn");
    if (disbandAgreeBtn)
      disbandAgreeBtn.addEventListener("click", () => castDisbandVote("agree"));
    if (disbandDisagreeBtn)
      disbandDisagreeBtn.addEventListener("click", () =>
        castDisbandVote("disagree"),
      );
    ...
    document.querySelectorAll(".diff-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        document
          ...
          .forEach((b) => b.classList.remove("diff-active"));
        btn.classList.add("diff-active");

        // Style update for active state
        document.querySelectorAll(".diff-btn").forEach((b) => {
          b.style.background = b.style.background.replace(/\d+\.\d+\)/, "0.1)"); // dim others
        });
        const color = btn.style.color;
        btn.style.background = `rgba(${color === "rgb(39, 196, 122)" ? "39,196,122" : color === "rgb(250, 204, 21)" ? "250,204,21" : "255,82,82"}, 0.15)`;
    ...
    if (leaveRoomResultBtn) {
      leaveRoomResultBtn.addEventListener("click", async () => {
        ...
        if (state.isHost) {
          if (
            confirm("Leaving will close the room for everyone. Are you sure?")
          ) {
          ...
          if (confirm("Are you sure you want to leave the room?")) {
            ...
            if (roomData) {
              for (let i = 0; i < roomData.teams.length; i++) {
                roomData.teams[i].players = roomData.teams[i].players.filter(
                  (p) => p.id !== state.myPlayerId,
                );
              }
              await supabase
                .from("rooms")
                .update({ teams: roomData.teams, last_update_time: Date.now() })
                .eq("id", state.roomId);
            }
            clearSession();
            window.location.reload();
    ...
    if (challengeCode && !sessionStorage.getItem(SESSION_KEYS.ROOM_ID)) {
      ...
      if (challengeHost) {
        ...
        banner.style =
          "background: rgba(216,177,91,0.15); border: 1px solid var(--accent); padding: 12px; border-radius: 12px; color: var(--accent); text-align: center; margin-bottom: 18px; font-weight: 700; font-size: 14px;";
        banner.innerHTML = `⚡ @${challengeHost.replace(/</g, "&lt;")} challenged you!`;
        $("join-room-code")
          .closest(".game-setup-card-wide")
          .insertBefore(
            banner,
            $("join-room-code").closest(".game-settings-row"),
          );
      }
  ...
  function persistSession() {
    if (state.roomId)
      sessionStorage.setItem(SESSION_KEYS.ROOM_ID, state.roomId);
    if (state.isHost) sessionStorage.setItem(SESSION_KEYS.IS_HOST, "true");
    if (state.myPlayerName)
      sessionStorage.setItem(SESSION_KEYS.PLAYER_NAME, state.myPlayerName);
  }
  ...
  async function tryAutoRejoin() {
    ...
    try {
      ...
      const isInRoom = roomData.teams.some((t) =>
        t.players.some((p) => p.id === savedPlayerId),
      );
      if (!isInRoom) {
        clearSession();
        return;
      ...
      state.myPlayerName =
        sessionStorage.getItem(SESSION_KEYS.PLAYER_NAME) || "Player";
      ...
      if (roomData.phase === "playing") {
        ...
        startCountdown();
        const myTeamId = roomData.teams[0].players.some(
          (p) => p.id === state.myPlayerId,
        )
          ? "a"
          : "b";
      ...
      $("waiting-room-code-display").innerHTML =
        `Room Code: <strong>${savedRoomId}</strong>`;
      if (state.isHost) {
  ...
  async function createRoom() {
    ...
    try {
      ...
      const playerName = (
        $("host-player-name").value.trim() || "Host"
      ).substring(0, 30);
      const teamId = $("host-team").value;
      state.myPlayerName = playerName;

      const initialStocks = clamp(
        parseInt($("initial-stocks").value) || 5,
        1,
        50,
      );
      const initialWorth = clamp(
        parseInt($("initial-worth").value) || 100,
        10,
        1000,
      );
      const matchDurationMin = clamp(
        parseInt($("match-duration").value) || 5,
        1,
        30,
      );
      ...
      state.match = {
        ...
        totalSells: 0,
        playerStates: {
          [state.myPlayerId]: createPlayerState(
            state.myPlayerId,
            initialStocks,
            initialWorth,
          ),
  ...
  async function joinRoom() {
    ...
    try {
      state.myPlayerId = getMyPlayerId();
      const playerName = (
        $("join-player-name").value.trim() || "Player"
      ).substring(0, 30);
      ...
      const isAlreadyInRoom = validated.teams.some((t) =>
        t.players.some((p) => p.id === state.myPlayerId),
      );
      if (!isAlreadyInRoom) {
        // Check team size limit
        if (
          validated.teams[targetTeamIdx].players.length >=
          (validated.max_team_size || MAX_TEAM_SIZE)
        ) {
          throw new Error(
            `Team ${validated.teams[targetTeamIdx].name} is full! (${validated.max_team_size || MAX_TEAM_SIZE} players max)`,
          );
        }

        ensurePlayerInOneTeam(
          validated.teams,
          state.myPlayerId,
          playerEntry,
          targetTeamIdx,
        ...
        validated.match.playerStates[state.myPlayerId] = createPlayerState(
          state.myPlayerId,
          validated.initial_stocks,
          validated.initial_worth,
        );
  ...
  function listenToRoom(code) {
    ...
    const handleData = (data) => {
      ...
      const amIStillInRoom = validated.teams.some((t) =>
        t.players.some((p) => p.id === state.myPlayerId),
      );
      ...
      if (
        ...
        dom.waitingRoom.hidden = false;
        $("waiting-room-code-display").innerHTML =
          `Room Code: <strong>${state.roomId}</strong>`;
        if (state.isHost) {
          $("start-game-btn").hidden = false;
        }
      }
        ...
        if (state.isHost) {
          ...
          if (tA > 0 && tB > 0 && tA !== tB) {
            $("waiting-message").textContent =
              "Teams must have an equal number of players to start.";
          } else if (!ready) {
            $("waiting-message").textContent =
    ...
    state.unsubscribeRoom = supabase
      .channel(`room-${code}`)
      .on(
        "postgres_changes",
        {
  ...
  async function switchTeam() {
    ...
    try {
      ...
      for (let i = 0; i < 2; i++) {
        const idx = validated.teams[i].players.findIndex(
          (p) => p.id === state.myPlayerId,
        );
        if (idx !== -1) {
      ...
      if (!playerEntry || fromTeam === -1) {
        console.error(
          "[SwitchTeam] Player not found in teams. myPlayerId:",
          state.myPlayerId,
          "Teams:",
          validated.teams,
        );
      ...
      if (
        validated.teams[toTeam].players.length >=
        (validated.max_team_size || MAX_TEAM_SIZE)
      ) {
  ...
  async function initiateDisbandVote() {
    if (!state.roomId) return;
    if (!confirm("Are you sure you want to start a vote to disband this room?"))
      return;
  ...
  async function castDisbandVote(voteType) {
    ...
    if (
      ...
      !roomData.match_settings.disband_vote
    )
      return;
  ...
  function renderWaitingRoom() {
    ...
    if (teamAHeader)
      teamAHeader.textContent = `(${state.teams[0].players.length}/${maxSize})`;
    if (teamBHeader)
      teamBHeader.textContent = `(${state.teams[1].players.length}/${maxSize})`;
      ...
      state.teams[0].players
        .map((p) => {
          ...
          const statusIndicator = isOnline
            ? ""
            : '<span style="color:var(--red);font-size:10px;margin-left:6px;">(Offline)</span>';
          const removeBtn =
            state.isHost && !isMe
              ? `<button class="remove-player-btn" data-id="${p.id}" style="background:transparent; border:none; color:var(--red); cursor:pointer; font-size:12px; margin-left:auto;" title="Remove Player">✖</button>`
              : "";
        ...
        .join("") ||
      '<div style="color:var(--muted);padding:12px;text-align:center;font-size:13px;">No players yet</div>';
  ...
  async function startGame() {
    ...
    if (
      state.teams[0].players.length === 0 ||
      state.teams[1].players.length === 0
    ) {
    ...
    try {
      ...
      roomData.teams.forEach((team) => {
        team.players.forEach((player) => {
          ...
          ps.currentQuestion = generateQuestion(chosenDifficulty);
          ps.questionNumber = 1;
      ...
      const voiceMode = $("waiting-room-voice-mode")
        ? $("waiting-room-voice-mode").value
        : "team";
  ...
  async function resetForNextMatch() {
    ...
    try {
      ...
      roomData.teams.forEach((team) => {
        team.players.forEach((player) => {
          playerStates[player.id] = createPlayerState(
            player.id,
            initialStocks,
            initialWorth,
          );
  ...
  function calculateMomentumMultiplier(streak) {
  ...
  async function handleAnswer(e) {
    ...
    try {
      ...
      const myState = JSON.parse(
        JSON.stringify(roomData.match.playerStates[state.myPlayerId]),
      );
      ...
      let availableShares =
        roomData.match.availableShares !== undefined
          ? roomData.match.availableShares
          : 5000;

      const volatilitySetting =
        (roomData.match_settings &&
          roomData.match_settings.market_volatility) ||
        "medium";
      let volMultiplier = 1.0;
      if (volatilitySetting === "low") volMultiplier = 0.5;
      else if (volatilitySetting === "high") volMultiplier = 2.0;

      const activeEvent = roomData.match.activeEvent;
      const eventActive = activeEvent && activeEvent.expiresAt > Date.now();

      if (eventActive) {
        if (activeEvent.type === "circuit_breaker") {
          myState.lastFeedback = {
            text: `<div class="dopamine-tag">CIRCUIT BREAKER ACTIVE</div><div class="dopamine-sub">Trading halted. Wait ${Math.ceil((activeEvent.expiresAt - Date.now()) / 1000)}s</div>`,
            className: "game-feedback feedback-flash-red",
            isHtml: true,
          };
          roomData.match.playerStates[state.myPlayerId] = myState;
          await supabase
            .from("rooms")
        ...
        if (
          activeEvent.type === "bull_run" &&
          (tradeAction === "b" || tradeAction === "+")
        ) {
          volMultiplier *= 3.0;
        }
        if (activeEvent.type === "volatility_spike") {
          volMultiplier *= 4.0;
        }
      }
      ...
      if (isCorrect) {
        ...
        if (tradeAction === "b" || tradeAction === "+") {
          if (myState.cash >= stockWorth) {
            ...
            if (lastTradeDir === "buy") momentumStreak++;
            else momentumStreak = 1;
            lastTradeDir = "buy";
            const momMult = calculateMomentumMultiplier(momentumStreak);
            roomData.match.momentumStreak = momentumStreak;
            roomData.match.lastTradeDir = lastTradeDir;
            ...
            const baseline = roomData.initial_worth || 100;
            const deviation = (stockWorth - baseline) / baseline;
            if (deviation > 0.4)
              stockWorth = stockWorth * (1 - (deviation - 0.4) * 0.05);
            if (deviation < -0.3)
              stockWorth = stockWorth * (1 + Math.abs(deviation + 0.3) * 0.05);
      ...
      if (
        ...
        activeEvent.expiresAt <= Date.now()
      ) {
        roomData.match.activeEvent = null;
      }
      myState.questionNumber += 1;
      myState.currentQuestion = generateQuestion(chosenDifficulty);
      myState.worth =
        Math.round((myState.cash + myState.stocks * stockWorth) * 100) / 100;
  ...
  function getTeamComputedData(teamIdx) {
    ...
    team.players.forEach((p) => {
      ...
      if (playerState) {
        const worth =
          Math.round(
            (playerState.cash + playerState.stocks * stockWorth) * 100,
          ) / 100;
  ...
  function findMyPlayerState() {
    return state.match.playerStates
      ? state.match.playerStates[state.myPlayerId]
      : null;
  }
  ...
  function scheduleNextEvent() {
    ...
    setTimeout(async () => {
      ...
      const { data: roomData } = await supabase
        ...
        .eq("id", state.roomId)
        .single();
      if (!roomData || roomData.phase !== "playing") return;
      if (
        roomData.match.activeEvent &&
        roomData.match.activeEvent.expiresAt > Date.now()
      ) {
        scheduleNextEvent();
        return;
      }
      ...
      updatedMatch.activeEvent = {
        ...
        startedAt: Date.now(),
        expiresAt: event.duration > 0 ? Date.now() + event.duration : 0,
      };
  ...
  function renderEventBanner() {
    ...
    const ae = state.match.activeEvent;
    ...
    if (!banner) {
      ...
      const qBox = document.getElementById("game-question-container");
      if (qBox && qBox.parentNode) {
        qBox.parentNode.insertBefore(banner, qBox);
      }
    }

    const remaining =
      ae.expiresAt > 0
        ? Math.ceil((ae.expiresAt - Date.now()) / 1000) + "s"
        : "";

    banner.style.cssText = `
            background: ${ae.color}18;
            border: 1px solid ${ae.color}60;
            border-radius: 8px;
            padding: 10px 16px;
            margin-bottom: 12px;
            display: flex;
            ...
            animation: eventPulse 1s ease-in-out infinite alternate;
        `;
    banner.innerHTML = `
            <div style="display:flex; align-items:center; gap:10px;">
                <span style="font-size:20px;">${ae.emoji}</span>
                <div>
                    <div style="font-size:11px; font-weight:700; letter-spacing:2px; color:${ae.color}; text-transform:uppercase;">${ae.label}</div>
                    <div style="font-size:12px; color:rgba(255,255,255,0.6); margin-top:2px;">${ae.description}</div>
                </div>
            </div>
            <div style="font-size:14px; font-weight:700; color:${ae.color};">${remaining}</div>
        `;
  }
  ...
  function renderArena() {
    ...
    if (availableSharesSpan && state.match) {
      availableSharesSpan.textContent =
        state.match.availableShares !== undefined
          ? state.match.availableShares
          : 5000;
    }
    ...
    if (dom.marketStatus) {
      const ms = state.match.momentumStreak || 0;
      let msText = "NEUTRAL";
      let msColor = "var(--t3)";
      if (ms >= 3) {
        msText = `BULL x${calculateMomentumMultiplier(ms)}`;
        msColor = "#27C47A";
      }
      if (ms <= -3) {
        msText = `BEAR x${calculateMomentumMultiplier(ms)}`;
        msColor = "#FF5252";
      }
    ...
    if ($("team-a-pl")) {
      ...
      $("team-a-pl").style.color =
        plA >= 0 ? "var(--success)" : "var(--danger)";
    }
    if ($("team-b-pl")) {
      $("team-b-pl").textContent = (plB >= 0 ? "+" : "") + plB.toFixed(1) + "%";
      $("team-b-pl").style.color =
        plB >= 0 ? "var(--success)" : "var(--danger)";
    }
  ...
  function endGameLocal() {
    ...
    const winner =
      teamAData.totalWorth >= teamBData.totalWorth ? teamAData : teamBData;
    const winnerName =
    ...
    dom.resultMvpWorth.textContent = mvp
      ? `Worth: ${mvp.worth.toFixed(2)}`
      : "";
    dom.resultWinnerName.textContent = isTie ? "TIE" : winnerName.toUpperCase();
    dom.resultWinnerWorth.textContent = isTie
      ? `Both: ${teamAData.totalWorth.toFixed(2)}`
      : `Worth: ${winner.totalWorth.toFixed(2)}`;
    ...
    if (dom.playAgainBtn) {
      dom.playAgainBtn.textContent = state.isHost
        ? "PLAY AGAIN"
        : "WAITING FOR HOST...";
    ...
    const matchData = {
      ...
      total_trades:
        ...
        teamBData.players.reduce((sum, p) => sum + (p.trades || 0), 0),
      ...
      supabase.auth
        ...
        .then(async ({ data: { session } }) => {
          if (!session || !session.user) {
            alert(
              "Debug: No authenticated session found. ELO will not be updated. Please sign in on the Live Desk tab.",
            );
            return;
          }
          ...
          try {
            ...
            const { data: profile, error: profileErr } = await supabase
              .from("profiles")
              .select("elo_score, peak_elo, matches_played, matches_won")
              .eq("id", userId)
              .single();

            if (profileErr) {
              alert(
                `Debug: Failed to fetch profile from DB: ${profileErr.message}`,
              );
              return;
            }
            ...
            if (profile) {
              ...
              if (updateError) {
                ...
                alert(
                  "Debug: Update succeeded but returned 0 rows. RLS blocked it or user missing.",
                );
              } else {
                alert(
                  `Debug: ELO Updated Successfully!\nResult: ${isTie ? "TIE" : wonParam ? "WIN" : "LOSS"}\nELO Change: ${eloChange}\nNew ELO: ${newElo}`,
                );
                console.log(
                  `Updated ELO for ${userId}: ${currentElo} -> ${newElo}`,
                );
              }
            }
          } catch (e) {
  ...
  function drawChart(canvas) {
    ...
    if (
      data.length > anim.displayData.length ||
      Math.abs(targetLast - currentLast) > 0.05
    ) {
      anim.pulse = 1.0;
      anim.pulseSize = Math.min(
        25,
        10 + Math.abs(targetLast - currentLast) * 2,
      );
    }

    // Sync array lengths for smooth transitions
    while (anim.displayData.length < data.length) {
      anim.displayData.push(
        anim.displayData[anim.displayData.length - 1] || data[data.length - 1],
      );
    }
    while (anim.displayData.length > data.length) {
    ...
    if (!anim.frameId) {
      const loop = () => {
        ...
        const noise =
          state.phase === "playing"
            ? Math.sin(time * 2.5) * 0.08 + Math.cos(time * 1.8) * 0.05
            : 0;
  ...
  function showShareCard(matchData) {
    ...
    document.getElementById("btn-download").onclick = () => {
      html2canvas(document.getElementById("victory-card"), { scale: 2 }).then(
        (canvas) => {
