...
def build_intelligence_batch(articles):
    """
    Process a list of articles in a single batched AI call.
    articles is a list of dicts: [{"title": t, "summary": s, "body": b, "link": l, "published_at": p, ...}]
    """
    from backend.openrouter_client import analyze_news_batch
    ...
    batch_payload = []
    for art in articles:
        initial_category = guess_category(art["title"], art["summary"])
    ...
    analyses = analyze_news_batch(batch_payload)
    if not analyses:
        raise Exception("AI batch analysis failed. Skipping batch.")

    results_map = {}
    for analysis in analyses:
        idx = analysis.get("article_index")
        if idx is None or idx >= len(articles) or idx < 0:
            continue
            
        art = articles[idx]
        initial_category = guess_category(art["title"], art["summary"])
        initial_assets = extract_initial_assets(art["title"], art["summary"])
        ...
        basic_analysis = analysis.get("executive_summary", strip_html(art["summary"]))
