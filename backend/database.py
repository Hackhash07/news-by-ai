    ...
    try:
        ...
        if res_oldest.data:
            ...
            if ids_to_delete:
                ...
                # ONLY delete signals that were never evaluated. Keep Correct/Incorrect/Neutral for Lifetime stats!
                supabase.table("signal_outcomes").delete().in_("news_id", ids_to_delete).in_("status", ["PENDING", "RETRY", "AWAITING_MARKET", "NO_DATA", "UNRESOLVABLE"]).execute()
        ...
        if remaining_excess > 0:
            ...
            if res_hard_oldest.data:
                ...
                if hard_ids_to_delete:
                    ...
                    supabase.table("signal_outcomes").delete().in_("news_id", hard_ids_to_delete).in_("status", ["PENDING", "RETRY", "AWAITING_MARKET", "NO_DATA", "UNRESOLVABLE"]).execute()
