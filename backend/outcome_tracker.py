...
SKIP_TICKERS = {
    ...
    "^TNX", "^IRX", "^TYX", "^FVX", "^VIX", "IT=F", "EXSZ.DE",
    "RHM.DE", "DX-Y.N", "SX8P", "SXPP"
}
    ...
    try:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as res:
                if res.status != 200:
                    logger.error(f"yfinance direct API failed for {yahoo_ticker}: HTTP {res.status}")
                    return None
...
def fetch_and_fill_outcomes():
    ...
    try:
        ...
        # Pull up to 10 rows where status in PENDING, RETRY, AWAITING_MARKET and eval_time <= NOW
