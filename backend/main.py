...
def api_admin_pending_signals():
    ...
    try:
        ...
        response = supabase.table("signal_outcomes").select("id, ticker, signal_direction, signal_timestamp, evaluation_time, status, news!inner(title)").gte("signal_timestamp", "2026-07-04T08:05:00Z").in_("status", ["PENDING", "RETRY", "NO_DATA", "UNRESOLVABLE"]).order("signal_timestamp", desc=True).limit(50).execute()
        return jsonify({"signals": response.data})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
